...
- 역할 분리 실행은 [`rule/rules/subagent-orchestration.md`](subagent-orchestration.md)에서 정의하고, root [`AGENTS.md`](../../AGENTS.md)에는 그 존재만 연결한다.
...
- 사용자가 명시적으로 구현, 변경, 생성, 수정, materialize를 요청하지 않았다면 분석, 질문, 리뷰, 설명 요청을 구현 요청으로 오인하지 않는다.
- 분석 전용 요청에서는 implementation cycle이나 파일 수정으로 넘어가지 않고, 필요하면 explorer 분석으로만 범위를 좁힌다.
- 구현 의도가 모호하면 먼저 질문하거나 분석으로 멈추고, 구현 요청을 추측으로 보강하지 않는다.
