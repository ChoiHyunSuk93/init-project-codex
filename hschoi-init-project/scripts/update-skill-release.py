#!/usr/bin/env python3
"""Update an installed hschoi-init-project skill from a GitHub release tag."""
...
DEFAULT_SKILL_PATH = "hschoi-init-project"
DEFAULT_REF = "latest"
...
def github_request(url: str) -> bytes:
    headers = {"User-Agent": "hschoi-init-project-updater"}
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Update an installed hschoi-init-project skill from a tagged GitHub release."
        )
    )
    parser.add_argument(
        "--skill-dir",
