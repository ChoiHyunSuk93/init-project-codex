...
- 안정적이고 상세한 동작 규칙은 `rule/*.md`로 옮긴다.
- local `AGENTS.md`는 local scope를 분명하게 할 때만 사용한다.

## 중복 금지

- 이유 없이 같은 규칙을 여러 위치에 반복해서 쓰지 않는다.
- 상세 규칙이 바뀌면 원본 rule 문서와 필요한 index 참조를 갱신하고, 동일한 수정 내용을 여러 파일에 복사하지 않는다.
- 저장소 전역의 구현 품질 기대치는 여러 instruction 파일에 흩뿌리지 말고 `rule/development-standards.md`나 더 좁은 local rule에 둔다.
...
- 각 skill은 관련 `rule/*.md` 문서를 참조형으로 연결하고, 저장소 공통 규칙을 skill 본문에 중복 작성하지 않는다.
