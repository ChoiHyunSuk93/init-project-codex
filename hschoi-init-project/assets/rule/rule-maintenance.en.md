...
- `rule/index.md` is the discovery point for authoritative rule documents.
- Indexed rule documents live under `rule/rules/` unless the user explicitly asks for a different structure.
- A rule document is not authoritative until it is listed in `rule/index.md`.
...
- Update `rule/index.md` in the same change whenever a rule document is added, deleted, renamed, or moved.
...
- Keep the control filename stable as `rule/index.md`.
...
- Do not leave copied rule text spread across `AGENTS.md`, guide docs, or implementation records when the source rule document is the authoritative location.
- When introducing a local-scope rule, add it to `rule/index.md` as a local entry and keep it under `rule/rules/` unless the user explicitly requested a different location.
