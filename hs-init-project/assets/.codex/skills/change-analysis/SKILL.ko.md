...
- 요구사항을 구현 요청과 분석 요청으로 구분한다.
- 영향 범위, acceptance criteria, non-goal, 위험 요소를 정리한다.
- planner 역할이나 분석 단계에서 참조형 근거를 정리할 때 우선 사용한다.
- 구현 자체를 대신하지 않고, repository rule을 본문에 복사하지 않는다.
