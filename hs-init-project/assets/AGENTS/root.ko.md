...
이 파일은 얇게 유지하고, [`rule/index.md`](rule/index.md)와 `rule/rules/` 아래의 상세 규칙 문서로 연결하는 데 사용한다.

## 이 파일의 역할

- 프로젝트 전역 지침의 경계를 정의한다.
- [`rule/index.md`](rule/index.md)와 `rule/rules/` 아래의 기준 규칙 문서로 연결한다.
- 상세 규칙이 들어가야 할 내용을 이 파일에 중복 작성하지 않는다.

## 규칙 모델

- [`rule/index.md`](rule/index.md)를 기준 규칙 문서의 탐색 시작점으로 취급한다.
- 프로젝트 구조를 바꾸거나 장기 보관 문서를 새로 만들기 전에 관련 `rule/rules/*.md` 문서를 읽는다.
- rule 문서를 추가, 삭제, 이름 변경, 이동할 때는 [`rule/rules/rule-maintenance.md`](rule/rules/rule-maintenance.md)를 기준으로 삼는다.
- rule 문서를 추가, 삭제, 이름 변경, 이동할 때는 [`rule/index.md`](rule/index.md)도 함께 갱신한다.
...
- 생성되는 `.codex/agents/*.toml`에서는 `model`과 `model_reasoning_effort`를 생략해 두 설정을 부모 agent에서 상속한다.
...
- exact 언어 규칙과 path 불변 조건은 [`rule/rules/language-policy.md`](rule/rules/language-policy.md)를 따른다.
