...
- 이 디렉토리는 사용자-facing 최종 구현 브리핑의 기록 공간이며, plan, delegated work, validation 작업 문서는 두지 않는다.

## 파일 규칙

- 초기화 단계에서 빈 카테고리 디렉토리나 placeholder 기록 문서를 미리 만들지 않는다.
- 구현 변경은 acceptance criteria와 필요한 검증이 `PASS`가 된 뒤 해당 카테고리에 새 기록 문서로 남긴다.
...
- plan이나 구현 기록만 있고 필요한 검증이 끝나지 않은 상태에서는 최종 요약을 만들거나 갱신하지 않는다.
...
- 역할별 작업 문서의 형식은 `subagents_docs/cycles/` 중심 규칙을 따른다.
