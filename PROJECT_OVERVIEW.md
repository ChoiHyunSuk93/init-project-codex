...
- 생성 구조는 제품 중립적인 규약, 얇은 agent 진입점, 프로젝트 요구사항 명세, 사람이 읽는 docs 진입점과 adaptive 작업 기록 계층을 기본으로 포함한다.
- 범용 custom subagent와 범용 starter skill은 생성하지 않되, host agent의 실제 작업 provenance는 roadmap과 필요 시 cycle에 누적한다.
...
- `PROJECT_OVERVIEW.md`를 요구사항 기준으로 만들고, `rule/index.md`를 규약 탐색 진입점으로 만든다.
- `docs/guide/README.md`, `docs/implementation/AGENTS.md`, `subagents_docs/AGENTS.md`, `subagents_docs/roadmap.md`를 기본 생성한다.
- 작업 크기와 모호성에 따라 main agent가 직접 수행하거나 필요한 경우에만 동적 subagent를 사용한다.
- 중간 이상 작업, work-sharing, 여러 handoff는 main-owned append-only cycle로 추적하고, 검증 완료 구현 결과는 category 기반 implementation briefing으로 누적한다.

## 요구사항

- fresh mode와 existing-project mode 모두 `PROJECT_OVERVIEW.md`, 문서 진입점, 작업 기록과 다섯 규약을 안전하게 생성해야 한다.
- fresh mode는 초기 사용자 요구사항을 기준으로 오버뷰와 roadmap을 완성하고, 부족한 정보는 검증 가능한 미해결 질문으로 남긴다.
- existing-project mode는 실제 source entrypoint와 주요 module, manifest/config, 기존 문서, 테스트/빌드 명령을 읽은 뒤 오버뷰, guide index, roadmap과 관련 rule을 관찰된 사실로 작성하거나 보강한다.
- template materialization 뒤 semantic completion validator를 통과해야 초기화 완료로 판정한다.
...
- 프로젝트 전용 agent, skill, release 절차는 생성 대상 프로젝트에서 별도 작업으로 정의한다.
...
- materializer는 결정론적 baseline만 만들고, 실제 source 분석과 semantic completion은 invoking agent가 수행한다.
